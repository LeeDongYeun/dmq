...
class ActivationQuantizer(nn.Module):
    ...
    # def get_ptf_scale_zero_point(self, x, num_scales=4):
    #     """ If you have enough GPU Memory, use this function.
    #     """
    ...
    #     scales = torch.tensor(scales, device=x.device).view(1, 1, 1, -1)  # Shape: [1, 1, 1, 4]
    #     zero_point = zero_point.view(1, -1, 1, 1)  # Shape: [1, C, 1, 1]

    #     # Expand x to match the scales dimensions for broadcasting
    #     x_expanded = x.unsqueeze(-1)  # Shape: [N, C, H, W, 1] (for 4D) or [N, C, H, 1] (for 3D)

    #     # Quantize data with different scales
    #     data_q = ((x_expanded / scales + zero_point).round().clamp(self.min_int, self.max_int) - zero_point) * scales
        
    #     # Calculate loss for each scale (4 scales in total)
    #     scores = ((x_expanded - data_q).abs().pow(2)).mean(dim=list(range(2, x_expanded.dim() - 1))) # Shape: [N, C, 4]
        
    #     # Find the best scale (min score) for each channel
    #     best_per_sample = scores.argmin(dim=-1)  # Shape: [N, C]

    #     best_index = torch.mode(best_per_sample, dim=0)[0].float() # Shape: [C]
    #     self.index_ratio = (best_per_sample == best_index).sum(0) / best_per_sample.size(0)
        
    #     best_index[self.index_ratio < self.ratio_threshold] = num_scales - 1
    #     scale_mask = 2 ** (best_index - num_scales + 1) # [C]
    #     # scale_mask = 2 ** best_index  # Shape: [C]

    #     # Reshape scale_mask to match x's shape for broadcasting
    #     x_shape = [1, -1] + (len(x.size()) - 2) * [1]
    #     delta = (scale * scale_mask.view(*x_shape))
    #     # delta = (scales.reshape(-1)[0] * scale_mask.view(*x_shape))
